...
static void __config(const struct io_handle *this)
{
    ...
    switch (this->dir) {
        case GPIO_OUTPUT:
            LL_GPIO_SetPinMode(this->gpio_port, this->gpio_pin, LL_GPIO_MODE_OUTPUT);
            break;
        case GPIO_INPUT:
            LL_GPIO_SetPinMode(this->gpio_port, this->gpio_pin, LL_GPIO_MODE_INPUT);
            break;
        case GPIO_ALTERNATE:
            LL_GPIO_SetPinMode(this->gpio_port, this->gpio_pin, LL_GPIO_MODE_ALTERNATE);
            if (this->gpio_pin < LL_GPIO_PIN_8) {
                LL_GPIO_SetAFPin_0_7(this->gpio_port, this->gpio_pin, this->func);
            } else {
                LL_GPIO_SetAFPin_8_15(this->gpio_port, this->gpio_pin, this->func);
            }
            break;
        case GPIO_ANALOG:
            LL_GPIO_SetPinMode(this->gpio_port, this->gpio_pin, LL_GPIO_MODE_ANALOG);
            break;
        default:
            return;
    }
    ...
    switch (this->otype) {
        case GPIO_PUSHPULL:
            LL_GPIO_SetPinOutputType(this->gpio_port, this->gpio_pin, LL_GPIO_OUTPUT_PUSHPULL);
            break;
        case GPIO_OPENDRAIN:
            LL_GPIO_SetPinOutputType(this->gpio_port, this->gpio_pin, LL_GPIO_OUTPUT_OPENDRAIN);
            break;
        default:
            return;
    }
    ...
    switch (this->pull) {
        case GPIO_PULL_NO:
            LL_GPIO_SetPinPull(this->gpio_port, this->gpio_pin, LL_GPIO_PULL_NO);
            break;
        case GPIO_PULL_UP:
            LL_GPIO_SetPinPull(this->gpio_port, this->gpio_pin, LL_GPIO_PULL_UP);
            break;
        case GPIO_PULL_DOWN:
            LL_GPIO_SetPinPull(this->gpio_port, this->gpio_pin, LL_GPIO_PULL_DOWN);
            break;
        default:
            return;
    }
    ...
    switch (this->speed) {
        case GPIO_SPEED_LOW:
            LL_GPIO_SetPinSpeed(this->gpio_port, this->gpio_pin, LL_GPIO_SPEED_FREQ_LOW);
            break;
        case GPIO_SPEED_MEDIUM:
            LL_GPIO_SetPinSpeed(this->gpio_port, this->gpio_pin, LL_GPIO_SPEED_FREQ_MEDIUM);
            break;
        case GPIO_SPEED_HIGH:
            LL_GPIO_SetPinSpeed(this->gpio_port, this->gpio_pin, LL_GPIO_SPEED_FREQ_HIGH);
            break;
        case GPIO_SPEED_VERY_HIGH:
            LL_GPIO_SetPinSpeed(this->gpio_port, this->gpio_pin, LL_GPIO_SPEED_FREQ_VERY_HIGH);
            break;
        default:
            return;
    }
                       ...
                       enum gpio_direction direction)
{
    ...
    */
    ...
    /* Find pinmask */
    uint32_t pinmask = 0U;
    for (uint32_t pin = io_handle->gpio_pin_first; pin <= io_handle->gpio_pin_last; pin <<= 1U) {
        pinmask |= pin;
    }
